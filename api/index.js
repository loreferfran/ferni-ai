...
app.post('/api/sales-page', async function(req, res) {
  try {
    ...
    var sys = 'Eres copywriter senior de respuesta directa. Redacta la PÁGINA DE VENTA REAL de un infoproducto, ÍNTEGRAMENTE en ' + language + ' — es el copy final que verá el comprador, no un documento interno. CERO mezcla de idiomas.' +
      ...
      '\nHonestidad estructural: autoridad = del método, jamás credenciales inventadas del autor; pruebaSocial nunca inventada (mantén el placeholder traducido + la señal de mercado real); urgencia SOLO si el borrador la trae y NUNCA convertirla en compromisos en vivo (calls, sesiones, coaching) — solo entregables digitales automáticos.' +
...
app.post('/api/niche-report', async function(req, res) {
  ...
  var reportSchema = JSON.stringify({
    ...
    hojaVenta: {
      ...
      urgencia: { tipo: 'lanzamiento | bonus-limitado | ninguna', texto: 'SOLO urgencia honesta Y AUTOMÁTICA que no exija trabajo humano recurrente de la autora: precio de lanzamiento con fecha, bonus DIGITAL adicional (PDF/checklist/plantilla), o plazo de acceso. PROHIBIDO: sesiones en vivo, calls grupales, coaching, comunidades moderadas o cualquier compromiso que la autora deba ejecutar en persona. Si no hay urgencia automática creíble: tipo "ninguna" y texto vacío. PROHIBIDO inventar escasez falsa' },
